...
(function f_set_prototpe_for_F_XY() {
    ...
    //pair, where .x = .y = n
    G.F_XY.f_same_nn = function (n) { return new G.F_XY(n, n); };
    ...
    //two cells:  this_n_xy and p_n_xy  are neighbours (DELTA = [1,0], [0,1], [-1,0], [0,-1])
    G.F_XY.prototype.f_contact = function (p) {
        var x = this.x - p.x, y = this.y - p.y; //it is (delta_x, delta_y)
        return (((x == 0) && (Math.abs(y) == 1)) || ((y == 0) && (Math.abs(x) == 1)));
    };

    //convert (this.x,this.y) in one string with "," separator
    G.F_XY.prototype.f_xy_to_string = function (accuracy_to_n_decimal_signs) {
        var str_x = G.MATH.f_n_to_string(this.x, accuracy_to_n_decimal_signs);
        var str_y = G.MATH.f_n_to_string(this.y, accuracy_to_n_decimal_signs);
        return (str_x + ',' + str_y);
    }
}());